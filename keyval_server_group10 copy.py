...
class KeyValueServicer(keyval_pb2_grpc.KeyValueServicer):
    ...
    def Read(self, request, context):
        ...
        if key in store and key in store[key]:
            temp = store[key]
            return keyval_pb2.ReadResponse(status=status, key=key, value=temp[key], current_version=temp['new_version'])
        else:
    ...
    def Write(self, request, context):
        ...
        if 'new_version' in temp and  current_version > 0 and current_version != temp['new_version']:
            error = f"Write aborted. Record version mismatch. Expected = {current_version}, Actual = {temp['new_version']}"
            return keyval_pb2.WriteResponse(status=keyval_pb2.Status(server_id=int(args.server_id), ok=False, error=error), key=key, new_version=temp['new_version'])
        elif 'new_version' not in temp and  current_version > 0 :
        ...
        return keyval_pb2.WriteResponse(status=status, key=key, new_version=temp['new_version'])
    ...
    def Delete(self, request, context):
        ...
        if temp['new_version'] != current_version:
            error = f"Delete aborted. Record version mismatch: Expected = {current_version}, Actual = {temp['new_version']}"
            return keyval_pb2.DeleteResponse(status=keyval_pb2.Status(server_id=int(args.server_id), ok=False, error=error))
        
        del store[key]
        
        return keyval_pb2.DeleteResponse(status=status, key=key, deleted_value=value, deleted_version=temp['new_version'])
    ...
    def List(self, request, context):
        ...
        for key in store.keys():
            ...
            entries.append({"key": key, 
                            "value":temp[key],
                            "current_version":temp['new_version']})
